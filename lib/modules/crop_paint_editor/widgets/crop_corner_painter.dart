...
class CropCornerPainter extends CustomPainter {
  ...
  void _drawCorners({
    ...
    if (!drawCircle) {
      ...
      /// Top-Left
      path.addRect(Rect.fromLTWH(_cropOffsetLeft - cornerThickness + 1,
          _cropOffsetTop - cornerThickness, length, width));
      path.addRect(Rect.fromLTWH(_cropOffsetLeft - cornerThickness,
          _cropOffsetTop - cornerThickness, width, length));

      /// Top line
      path.addRect(Rect.fromLTWH(
          _cropOffsetLeft - 1, _cropOffsetTop - 1, cropRect.width, 1));
      // Top center brick, width equal to the cornerLength, thinckness equal to the cornerThickness
      path.addRect(
        Rect.fromLTWH(
          _cropOffsetLeft + cropRect.width / 2 - length / 2,
          _cropOffsetTop - cornerThickness,
          cornerLength,
          cornerThickness,
        ),
      );
      ...
      /// Top-Right
      path.addRect(Rect.fromLTWH(_cropOffsetRight - length + cornerThickness,
          _cropOffsetTop - 3, length, width));
      path.addRect(Rect.fromLTWH(_cropOffsetRight - width + cornerThickness,
          _cropOffsetTop - 3, width, length));

      /// Right line
      path.addRect(Rect.fromLTWH(
          _cropOffsetRight, _cropOffsetTop - 1, 1, cropRect.height));

      /// Right center brick
      path.addRect(
        Rect.fromLTWH(
          _cropOffsetRight,
          _cropOffsetTop + cropRect.height / 2 - length / 2,
          cornerThickness,
          cornerLength,
        ),
      );
      ...
      /// Bottom-Left
      path.addRect(Rect.fromLTWH(0 + _cropOffsetLeft - cornerThickness,
          _cropOffsetBottom - width + cornerThickness, length, width));
      path.addRect(Rect.fromLTWH(0 + _cropOffsetLeft - cornerThickness,
          _cropOffsetBottom - length + cornerThickness, width, length));

      /// Bottom line
      path.addRect(Rect.fromLTWH(
          _cropOffsetLeft - 1, _cropOffsetBottom, cropRect.width, 1));
      ...
      /// Left line
      path.addRect(Rect.fromLTWH(
          _cropOffsetLeft - 1, _cropOffsetTop - 1, 1, cropRect.height));

      /// Left center brick
      path.addRect(
        Rect.fromLTWH(
          _cropOffsetLeft - cornerThickness,
          _cropOffsetTop + cropRect.height / 2 - length / 2,
          cornerThickness,
          cornerLength,
        ),
