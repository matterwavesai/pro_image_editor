...
import 'package:pro_image_editor/modules/main_editor/utils/layer_interaction_manager.dart';
import 'package:pro_image_editor/modules/zoomable_main_editor/utils/crop_area_history.dart';
...
class ZoomableMainEditor extends StatefulWidget
  ...
  factory ZoomableMainEditor.memory(
    Uint8List byteArray, {
    Key? key,
  ...
  factory ZoomableMainEditor.file(
    File file, {
    Key? key,
    required CropRotateEditorInitConfigs initConfigs,
  }) {
  ...
  factory ZoomableMainEditor.asset(
    String assetPath, {
    Key? key,
    required CropRotateEditorInitConfigs initConfigs,
  }) {
  ...
  factory ZoomableMainEditor.network(
    String networkUrl, {
    Key? key,
    required CropRotateEditorInitConfigs initConfigs,
  }) {
  ...
  factory ZoomableMainEditor.autoSource({
    ...
    if (byteArray != null) {
      return ZoomableMainEditor.memory(
        byteArray,
        key: key,
        initConfigs: initConfigs,
      );
  ...
  @override
  State<ZoomableMainEditor> createState() => ZoomableMainEditorState();
}
...
class ZoomableMainEditorState extends State<ZoomableMainEditor>
    with
        TickerProviderStateMixin,
        ImageEditorConvertedConfigs,
        ImageEditorConvertedCallbacks,
        StandaloneEditorState<ZoomableMainEditor, CropRotateEditorInitConfigs>,
        ExtendedLoop,
        CropAreaHistory {
  ...
  final StreamController _layerStreamController = StreamController();
  ...
  Future<void> done() async {
    ...
    for (final layer in _layers) {
      if (layer is PaintingLayerData) {
        final pixelRatio = imageInfos!.pixelRatio;
        // need to consider pixel ratio
        final rect = Rect.fromCenter(
            center: (layer.offset + Offset(_imgWidth / 2, _imgHeight / 2)) *
                pixelRatio,
            width: layer.rawSize.width * pixelRatio,
            height: layer.rawSize.height * pixelRatio);
  ...
  void _onScaleStart(ScaleStartDetails details) {
    ...
    debugPrint('onScaleStart: $_selectedLayerIndex');
    // if (_selectedLayerIndex != -1) {
    //   // update the position of selected layer
    //   _layerInteractionManager.determineResizeMode(
    //     detail: details,
    //     activeLayer: _layers[_selectedLayerIndex],
    //     realHitPoint: _getRealHitPoint(
    //         zoom: userScaleFactor, position: details.localFocalPoint),
    //   );
    // }
    ...
    if (!_scaleStarted) {
      ...
      if (!isDesktop) {
        // no need to crop in main editor
    ...
    if (details.pointerCount == 2) {
      ...
      if (_currentCropAreaPart != CropAreaPart.none &&
        ...
        if (_selectedLayerIndex != -1) {
          // update the position of selected layer
          final updatedLayer =
              _layerInteractionManager.calculateMovementAndResize(
            activeLayer: _layers[_selectedLayerIndex],
            context: context,
            detail: details,
            realHitPoint: _getRealHitPoint(
          ...
          _layers[_selectedLayerIndex] = updatedLayer;

          _layerStreamController.add(null);
          // setState(() {});
        } else {
  ...
  void _onScaleEnd(ScaleEndDetails details) async {
    ...
    if (_selectedLayerIndex != -1) {
      // update the position of selected layer
      _layerInteractionManager.resetResizeMode();
    }
  ...
  Widget _buildRotationTransform({required Widget child}) {
          ...
          return Stack(
            ...
            children: [
              ...
              if (cropRotateEditorConfigs.transformLayers && layers != null)
                StreamBuilder(
                    stream: _layerStreamController.stream,
                      ...
                      return ClipRRect(
                        ...
                        child: Transform.scale(
                          ...
                          child: Stack(
                            ...
                            children: _layers.asMap().entries.map((layerItem) {
                              final index = layerItem.key;
                              final data = layerItem.value;
                              // return Container(
                              //     width: 200, height: 200, color: Colors.red);
                              return LayerWidget(
                                configs: configs,
                                editorCenterX:
                                    transformHelper.editorBodySize.width / 2,
                                editorCenterY:
                                    transformHelper.editorBodySize.height / 2,
                                layerData: data,
                                enableHitDetection: true,
                                isInteractive: false,
                                onTap: (layer) {},
                                onTapDown: (event) {
                                  debugPrint('onTapDown $index');
                                  ...
                                  _layerInteractionManager.determineResizeMode(
                                    ...
                                    realHitPoint: event.localPosition -
                                        const Offset(7, 7),
                                  );
                                },
                                onTapUp: () {
